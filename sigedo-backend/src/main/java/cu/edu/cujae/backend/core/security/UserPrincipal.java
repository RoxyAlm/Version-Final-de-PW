...
//	private String fullName;
	...
	public UserPrincipal(String id, String password, boolean active,
...
//		this.identification = identification;
	...
	public static UserPrincipal create(UserDto user) {
		...
		try {
			Collection<String> roleNames = (Collection<String>) user.getRole();
			authorities = AuthorityUtils.createAuthorityList(roleNames.toArray(new String[0]));
		} catch (Exception e) {
			authorities = Collections.singletonList(new SimpleGrantedAuthority("ROLE_USER"));

		}
		return new UserPrincipal(user.getId(), user.getPassword(), true, authorities);
	}
